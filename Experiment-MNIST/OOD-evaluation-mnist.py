...
ood_ds = D.FashionMNIST(
    root="./data",
    train=False,
    download=True,
    transform=T.Compose([
        T.ToTensor(),                   # now shape is (1, 28, 28)
        T.Lambda(lambda x: x.repeat(3, 1, 1)),  # (3, 28, 28)
        T.Resize((32, 32)),             # optional
...
ood_entropy = []
for xb, _ in torch.utils.data.DataLoader(ood_ds, batch_size=256):
